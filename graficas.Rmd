...
g2 <- ggplot(clientes_df, aes(x = x, y = y, fill = Atención)) +
...
Considerando poleo. Grafica con poleo
```{r}
y <- sort(sapply(1:101, function(i) datos %>% filter(polling == "Si", cliente==i) %>% nrow()))
y <- 100*(y/sum(y))
clientes_df <- data_frame(x = 1:101, y = y, Atención = factor(ifelse(abs(y-100/101) < .15, "Justo", "Injusto")))
g3 <- ggplot(clientes_df, aes(x = x, y = y, fill = Atención)) +
  geom_bar(stat="identity", width = 1, alpha = 0.95, col = 'gray') +
  scale_y_continuous(breaks = round(seq(0, (max(y) + 0.02), by = 0.1), 2)) +
  scale_color_manual(labels = c("T999", "T888"), values = c("blue", "red")) +
  labs(title = "Atención por Clientes") + xlab("Clientes") + ylab("Proporción de atencion por Clientes %") + theme_bw()
...
Grafica sin poleo
```{r}
y <- sort(sapply(1:101, function(i) datos %>% filter(polling == "No", cliente==i) %>% nrow()))
y <- 100*(y/sum(y))
clientes_df <- data_frame(x = 1:101, y = y, Atención = factor(ifelse(abs(y-100/101) < .15, "Justo", "Injusto")))
g4 <- ggplot(clientes_df, aes(x = x, y = y, fill = Atención)) +
  geom_bar(stat="identity", width = 1, alpha = 0.95, col = 'gray') +
  scale_y_continuous(breaks = round(seq(0, (max(y) + 0.02), by = 0.1), 2)) +
  scale_color_manual(labels = c("T999", "T888"), values = c("blue", "red")) +
  labs(title = "Atención por Clientes") + xlab("Clientes") + ylab("Proporción de atencion por Clientes %") + theme_bw()
...
```{r}
g5 <- ggplot() + geom_histogram(aes(x = datos$dif_tiempos, y = ..density.., is.na = FALSE), col = 'black', alpha = 0.2, fill='blue') + 
...
```{r}
g6 <- ggplot(data = summary_df %>% filter(!is.na(tiempo)), aes(x = tasa, y = tiempo, col = polling)) + geom_line() + geom_point() +
...
g7 <- ggplot(num_df, aes(x = id, y = valor, col = variable)) + geom_point()
g7 
```
