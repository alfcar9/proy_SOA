...
throughput_df <- throughput_df %>% mutate(tasainv = 101/tasa*1000 )
throughput_df[13,3] <- 5110
```

Gráfica de rendimiento.
```{r met1, cache = TRUE}
g1 <- ggplot(data = throughput_df, aes(x = tasainv, y = tasa_recibido, col = polling)) + geom_line(size = 0.3) +
  geom_point() + labs(title = "Throughput", x = "Tasa", y = "Output") + theme_bw()
g1
```
